					   ...
					   bool have_record_arg)
{
	...
	for (i = 0; i < ncolumns; ++i)
	{
		...
		if (
#ifndef JSON_GENERIC
			(jtype == JSONOID && (hashentry == NULL || hashentry->isnull)) ||
			(jtype == JSONBOID && (v == NULL || v->type == jbvNull)))
#else
			(v == NULL || v->type == jbvNull))
#endif
		{
			/*
...
jsonb_pretty(PG_FUNCTION_ARGS)
{
	...
	JsonbToCStringIndent(str, JsonRoot(jb), JsonGetSize(jb));
...
jsonb_canonical(PG_FUNCTION_ARGS)
{
	...
	JsonbToCStringCanonical(str, JsonRoot(jb), JsonGetSize(jb));
